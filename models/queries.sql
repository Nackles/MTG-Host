...
from game g
left join (select id, name from player) as player1_sub on g.player1_id = player1_sub.id
left join (select id, name from player) as player2_sub on g.player2_id = player2_sub.id
left join (select id, name from player) as player3_sub on g.player3_id = player3_sub.id
left join (select id, name from player) as player4_sub on g.player4_id = player4_sub.id
left join (select id, name from player) as winner_sub on g.winner_id = winner_sub.id
;
-- QUERY TO GET WIN/LOSS RESULTS
select player.name as MagicPlayer, player.img_link as icon,
(select @X := count(*) from result where player.id = result.player_id and result.outcome = 'W') as W,
(select @Y := count(*) from result where player.id = result.player_id and result.outcome = 'L') as L,
...
from result r left join player p on  r.player_id = p.id
group by 1,2
order by 1,2 desc;
